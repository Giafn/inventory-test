<x-app-layout>
        ...
        <table class="table-auto w-full border-collapse border" id="dataTable">
            <thead>
                <tr>
                    ...
                    <th class="border px-4 py-2 dark:border-dark-eval-1">Sale By</th>
                    @endif
    ...
    <x-modal name="add-modal" :show="$errors->addItem->isNotEmpty()" :maxWidth="'2xl'" focusable>
        <form method="post" action="{{ route('sales.upsert') }}" class="p-6">
            @csrf
            @method('post')
    ...
    <x-modal name="edit-modal" :show="$errors->editItem->isNotEmpty()" focusable>
        <form method="post" class="p-6" x-data="" action="{{ route('sales.upsert') }}">
            @csrf
            @method('post')
            <h2 class="text-lg font-medium">
                Edit Sale Item
